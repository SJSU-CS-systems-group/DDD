    ...
    fun transportTransferToUsb(context: Context) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                ...
                //val bundlesToTransfer: MutableList<File> = sourceDir.walk().filter(File::isFile).toList() as MutableList<File>
                        ...
                        if (targetFile != null) {
                            context.contentResolver.openOutputStream(targetFile.uri)?.use { outputStream ->
                                Files.newInputStream(bundle.toPath())?.use { inputStream ->
                                    ...
                                    logger.log(INFO, "Copying from: ${bundle} to ${targetFile.uri}")
                                    inputStream.copyTo(outputStream)
                                }
                            }
                        }

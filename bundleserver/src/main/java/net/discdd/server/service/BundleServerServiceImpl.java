...
public class BundleServerServiceImpl extends BundleServerServiceGrpc.BundleServerServiceImplBase {
    ...
    @Override
    public void bundleInventory(BundleInventoryRequest request, StreamObserver<BundleInventoryResponse> responseObserver) {
        var serverBundlesOnTransport = request.getBundlesFromServerOnTransportList().stream()
        ...
        try {
            var bundlesToExchange = bundleTransmission.generateBundlesForTransmission(request.getSender(), serverBundlesOnTransport);

...
3. **test-canary** — calls the reusable `client-test.yml` workflow with `environment: canary`. Uses the nightly CLI jar as the backward-compat jar and the newly built CLI jar for the new-jar exchange. See `client-test.yml` for details.

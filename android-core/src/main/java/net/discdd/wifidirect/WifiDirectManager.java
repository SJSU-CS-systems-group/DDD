...
public class WifiDirectManager {
    ...
    public enum WifiDirectEventType {
        WIFI_DIRECT_MANAGER_INITIALIZED, WIFI_DIRECT_MANAGER_PEERS_CHANGED,
        WIFI_DIRECT_MANAGER_CONNECTION_CHANGED, WIFI_DIRECT_MANAGER_GROUP_INFO_CHANGED,
        WIFI_DIRECT_MANAGER_DISCOVERY_CHANGED, WIFI_DIRECT_MANAGER_DEVICE_INFO_CHANGED
    }
            ...
            if (action != null) switch (action) {
                case WIFI_P2P_STATE_CHANGED_ACTION -> {
                    ...
                    if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED) {
                        ...
                        manager.requestPeers(channel, peers -> discoveredPeers =
                                new HashSet<>(peers.getDeviceList()));
                    } else {
                ...
                case WIFI_P2P_PEERS_CHANGED_ACTION -> {
                    ...
                    try {
                        var newPeerList = intent.getParcelableExtra(EXTRA_P2P_DEVICE_LIST,
                                                                    WifiP2pDeviceList.class);
                        if (newPeerList != null) {
                            discoveredPeers = new HashSet<>(newPeerList.getDeviceList());
                        }
                        notifyActionToListeners(
                                WifiDirectEventType.WIFI_DIRECT_MANAGER_PEERS_CHANGED);
                ...
                case WIFI_P2P_CONNECTION_CHANGED_ACTION -> {
                    ...
                    if (networkInfo != null && networkInfo.isConnected()) {
                        ...
                        if ((groupOwnerAddress != null &&
                                !groupOwnerAddress.equals(newGroupOwnerAddress) ||
                                (groupOwnerAddress == null && newGroupOwnerAddress != null))) {
                            groupOwnerAddress = newGroupOwnerAddress;
                            groupInfoChanged = true;
                            notifyActionToListeners(
                                    WifiDirectEventType.WIFI_DIRECT_MANAGER_CONNECTION_CHANGED);
                        }
                    } else {
                        if (groupOwnerAddress != null) {
                    ...
                    if (groupInfoChanged) {
                        notifyActionToListeners(
                                WifiDirectEventType.WIFI_DIRECT_MANAGER_GROUP_INFO_CHANGED);
                    }
                }
                ...
                case WIFI_P2P_DISCOVERY_CHANGED_ACTION -> {
                    ...
                    notifyActionToListeners(
                            WifiDirectEventType.WIFI_DIRECT_MANAGER_DISCOVERY_CHANGED);
                }
            }
        }
    }

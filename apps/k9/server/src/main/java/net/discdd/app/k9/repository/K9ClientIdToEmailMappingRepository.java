...
public interface K9ClientIdToEmailMappingRepository extends CrudRepository<K9ClientIdToEmailMapping, String> {
    @Transactional
    @Modifying
    @Query(value = "UPDATE k9client_id_to_email_mapping SET client_id = :newClientId WHERE email = :email", nativeQuery = true)
    void updateClientId(@Param("newClientId")
    String newClientId, @Param("email")
    String email);
